/**
 * Gedeelde CA-module voor server-side fetches naar nederlanders.fr (NING).
 *
 * NING serveert een incomplete TLS-keten (curl error 60 / "unable to get issuer
 * certificate"). We BREIDEN de CA-set UIT — systeem-roots + de twee inline
 * certificaten — zodat elke keten die NING mogelijk serveert (RSA naar een
 * standaardroot, óf ECDSA/YR2) verifieerbaar blijft. Nooit vervangen, anders
 * breekt de andere kant.
 *
 * De keten staat INLINE (publieke certificaten), zodat hij niet afhangt van
 * file-tracing bij de Vercel-deploy (een los .pem werd niet mee-gedeployed →
 * ENOENT). Bron van waarheid blijft certs/ning-ca-bundle.pem; die wordt actueel
 * gehouden door .github/workflows/nlfr-bereik-test.yml. Werk bij een
 * keten-wijziging zowel dat bestand als de constante hieronder bij.
 *
 * Bestandsnaam begint met "_" zodat Vercel dit niet als route deployt, maar het
 * wél importeerbaar is vanuit de echte endpoints.
 */

import https from 'node:https';
import tls from 'node:tls';

// Intermediate: C = US, O = Let's Encrypt, CN = YR2
// Root:         C = US, O = ISRG, CN = Root YR
export const NING_CA_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

// De losse certificaten, om ze bij tls.rootCertificates te voegen.
export const NING_CA_CERTS =
  NING_CA_PEM.match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/g) || [];

// Systeem-roots + inline certs (uitbreiden, niet vervangen).
export const NING_CA_SET = [...tls.rootCertificates, ...NING_CA_CERTS];

// Kant-en-klare https-agent voor alle NING-fetches.
export const ningAgent = new https.Agent({ ca: NING_CA_SET, keepAlive: false });

/**
 * Gedeelde fetch bovenop ningAgent (Node https.request — undici/global fetch
 * kan geen https-agent met ca meekrijgen). Volgt redirects. Geeft terug:
 * { status, ok, length, body, finalUrl }.
 */
export function ningFetch(url, { timeoutMs = 5000, headers = {}, maxRedirects = 5 } = {}) {
  return new Promise((resolve, reject) => {
    const doReq = (targetUrl, redirectsLeft) => {
      let u;
      try {
        u = new URL(targetUrl);
      } catch (e) {
        return reject(e);
      }
      const req = https.request(
        {
          method: 'GET',
          hostname: u.hostname,
          port: u.port || 443,
          path: u.pathname + u.search,
          servername: u.hostname,
          agent: ningAgent,
          headers,
        },
        (res) => {
          const status = res.statusCode || 0;
          const loc = res.headers.location;
          if (status >= 300 && status < 400 && loc && redirectsLeft > 0) {
            res.resume();
            let next;
            try {
              next = new URL(loc, targetUrl).toString();
            } catch (e) {
              return reject(e);
            }
            return doReq(next, redirectsLeft - 1);
          }
          let data = '';
          res.setEncoding('utf8');
          res.on('data', (d) => { data += d; });
          res.on('end', () => resolve({
            status,
            ok: status >= 200 && status < 300,
            length: data.length,
            body: data,
            finalUrl: targetUrl,
          }));
        }
      );
      req.setTimeout(timeoutMs, () => { req.destroy(new Error(`Timeout (>${timeoutMs}ms)`)); });
      req.on('error', reject);
      req.end();
    };
    doReq(url, maxRedirects);
  });
}
